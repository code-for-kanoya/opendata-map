 ...
 * データセットデータCSV取込
 */
class DatasetFireIrrigationFacilitiesCSVImporter
{
    /**
     * ファイル読込用リソースファイルパス
     *
    ...
    public function importData($dirName, $data, $extension = 'csv')
    {
        ...
        // 既存登録のデリート
        $record = DataSetFireIrrigationFacilities::where('code', $code)->delete();
            ...
            if ($lineNumber === 0) {
                if ($extension !== 'csv') {
                    ...
                    for ($j = 0; $j < count($line); $j++) {
                        switch ($line[$j]) {
                            case DatasetFireIrrigationFacilitiesCSVTitleNumber::TYPE:
                                $titleCount += 1;
                                break;
                            default:
                                break;
                    ...
                    if ($titleCount < 1) {
                        ...
                        // 必須列タイトルが足りない為、スキップ
                        $message = DatasetCSVTitleName::FIRE_IRRIGATION_FACILITIES." : {$dirName}-> format no match!";
                        \Log::error($message);
                        return;
                    }
                }
                ...
                try {
                    ...
                    if (count($line) < count($csvTitleTable)) {
                        // データ読込エラー
                        throw new \Exception(DatasetCSVTitleName::FIRE_IRRIGATION_FACILITIES." : {$dirName}-> {$lineNumber} th line's data broken!", 1);
                    }
                    ...
                    if ($column_check === false) {
                        ...
                        \Log::info(DatasetCSVTitleName::FIRE_IRRIGATION_FACILITIES." : {$dirName}-> {$lineNumber} th line's data skip!");
                        $lineNumber += 1;
                        continue;
                    }

                    // データ行を処理(登録)
                    $record = new DataSetFireIrrigationFacilities();

                    // 各列のデータを取得
                    $record->code = Arr::get($line, Arr::get($csvTitleTable, DatasetFireIrrigationFacilitiesCSVTitleNumber::CODE, '')); // 都道府県コード又は市区町村コード
                    if (empty($record->code) || (strlen($record->code) < 6)) {
                        $record->code = $code; // 都道府県コード又は市区町村コード
                    }
                    $record->no = Arr::get($line, Arr::get($csvTitleTable, DatasetFireIrrigationFacilitiesCSVTitleNumber::NUMBER, '')); // NO
                    $record->prefecture_name = Arr::get($line, Arr::get($csvTitleTable, DatasetFireIrrigationFacilitiesCSVTitleNumber::PREFECTURE_NAME, '')); // 都道府県名
                    $record->municipality_name = Arr::get($line, Arr::get($csvTitleTable, DatasetFireIrrigationFacilitiesCSVTitleNumber::MUNICIPALITY_NAME, '')); // 市区町村名
                    if (empty($record->municipality_name)) {
                        $record->municipality_name = $codeName; // 市区町村名
                    }
                    $record->type = $line[$csvTitleTable[DatasetFireIrrigationFacilitiesCSVTitleNumber::TYPE]]; // 種別
                    $record->address = Arr::get($line, Arr::get($csvTitleTable, DatasetFireIrrigationFacilitiesCSVTitleNumber::ADDRESS, '')); // 住所
                    $record->address_detail = Arr::get($line, Arr::get($csvTitleTable, DatasetFireIrrigationFacilitiesCSVTitleNumber::ADDRESS_DETAIL, '')); // 方書
                    $record->latitude = Arr::get($line, Arr::get($csvTitleTable, DatasetFireIrrigationFacilitiesCSVTitleNumber::LATITUDE, '')); // 緯度
                    if (empty($record->latitude) || !is_numeric($record->latitude)) {
                        $record->latitude = 0; // 緯度
                    }
                    $record->longitude = Arr::get($line, Arr::get($csvTitleTable, DatasetFireIrrigationFacilitiesCSVTitleNumber::LONGITUDE, '')); // 経度
                    if (empty($record->longitude) || !is_numeric($record->longitude)) {
                        $record->longitude = 0; // 経度
                    }
                    $record->caliber = Arr::get($line, Arr::get($csvTitleTable, DatasetFireIrrigationFacilitiesCSVTitleNumber::CALIBER, '')); // 口径
                    $record->note = Arr::get($line, Arr::get($csvTitleTable, DatasetFireIrrigationFacilitiesCSVTitleNumber::NOTE, '')); // 備考
