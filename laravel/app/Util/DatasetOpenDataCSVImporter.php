 ...
 * データセットデータCSV取込
 */
class DatasetOpenDataCSVImporter
{
    /**
     * ファイル読込用リソースファイルパス
     *
    ...
    public function importData($dirName, $data, $extension = 'csv')
    {
        ...
        // 既存登録のデリート
        $record = DataSetOpenData::where('code', $code)->delete();
            ...
            if ($lineNumber === 0) {
                if ($extension !== 'csv') {
                    ...
                    for ($j = 0; $j < count($line); $j++) {
                        switch ($line[$j]) {
                            case DatasetOpenDataCSVTitleNumber::NAME:
                            case DatasetOpenDataCSVTitleNumber::FORMAT:
                            case DatasetOpenDataCSVTitleNumber::CLASSIFICATION:
                            case DatasetOpenDataCSVTitleNumber::LAST_UPDATE_DATE:
                                $titleCount += 1;
                                break;
                            default:
                                break;
                ...
                try {
                    ...
                    if (count($line) < count($csvTitleTable)) {
                        // データ読込エラー
                        throw new \Exception(DatasetCSVTitleName::OPEN_DATA." : {$dirName}-> {$lineNumber} th line's data broken!", 1);
                    }
                    ...
                    if ($column_check === false) {
                        ...
                        \Log::info(DatasetCSVTitleName::OPEN_DATA." : {$dirName}-> {$lineNumber} th line's data skip!");
                        $lineNumber += 1;
                        continue;
                    }

                    // データ行を処理(登録)
                    $record = new DataSetOpenData();
                    ...
                    $record->prefecture_name = Arr::get($line, Arr::get($csvTitleTable, DatasetOpenDataCSVTitleNumber::PREFECTURE_NAME, '')); // 都道府県名
                    $record->municipality_name = Arr::get($line, Arr::get($csvTitleTable, DatasetOpenDataCSVTitleNumber::MUNICIPALITY_NAME, '')); // 市区町村名
                    if (empty($record->municipality_name)) {
                        $record->municipality_name = $codeName; // 市区町村名
                    }
                    $record->name = $line[$csvTitleTable[DatasetOpenDataCSVTitleNumber::NAME]]; // データ名称
                    $record->overview = Arr::get($line, Arr::get($csvTitleTable, DatasetOpenDataCSVTitleNumber::OVERVIEW, '')); // データ概要
                    $record->format = $line[$csvTitleTable[DatasetOpenDataCSVTitleNumber::FORMAT]]; // データ形式
                    $record->classification = $line[$csvTitleTable[DatasetOpenDataCSVTitleNumber::CLASSIFICATION]]; // 分類
                    $record->update_frequency = Arr::get($line, Arr::get($csvTitleTable, DatasetOpenDataCSVTitleNumber::UPDATE_FREQUENCY, '')); // 更新頻度
                    $record->url = Arr::get($line, Arr::get($csvTitleTable, DatasetOpenDataCSVTitleNumber::URL, '')); // URL
                    $record->existence_api_support = Arr::get($line, Arr::get($csvTitleTable, DatasetOpenDataCSVTitleNumber::EXISTENCE_API_SUPPORT, '')); // API対応有無
                    $record->license = Arr::get($line, Arr::get($csvTitleTable, DatasetOpenDataCSVTitleNumber::LICENSE, '')); // ライセンス
                    $record->regist_date = Arr::get($line, Arr::get($csvTitleTable, DatasetOpenDataCSVTitleNumber::REGIST_DATE, '')); // 登録日
                    $record->last_update_date = $line[$csvTitleTable[DatasetOpenDataCSVTitleNumber::LAST_UPDATE_DATE]]; // 最終更新日
                    $record->note = Arr::get($line, Arr::get($csvTitleTable, DatasetOpenDataCSVTitleNumber::NOTE, '')); // 備考
